...
class RouteFinder {
   ...
   * @return {@link EscapeNode} wrapping the target Node with a pointer
   * that will enable us to construct a route between the target and
   * the start position of our route.
   */
  ...
  /**
   * Checks the current position's neighbours to either add them to the list of tiles to be considered while
   * looking for a route, or if they've already been considered, check that the stored cost isn't greater than
   * the cost of the current route.
   * @param neighbours Set of {@link Node} objects representing the current positions non-wall neighbours.
   */
